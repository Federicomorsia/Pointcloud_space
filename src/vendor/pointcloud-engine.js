...
export const createPointcloudEngine = (inputOptions = {}) => {
	...
	const hideNonBloomed = (object3D) => {
		if ((object3D.isPoints || object3D.isMesh) && !bloomLayer.test(object3D.layers)) {
			hiddenVisibility[object3D.uuid] = object3D.visible;
			object3D.visible = false;
		}
	};

	const restoreHiddenVisibility = (object3D) => {
		if (Object.prototype.hasOwnProperty.call(hiddenVisibility, object3D.uuid)) {
			object3D.visible = hiddenVisibility[object3D.uuid];
			delete hiddenVisibility[object3D.uuid];
		}
	};
	...
	const renderScene = () => {
		if (engineConfig.bloomEnabled) {
			...
			scene.traverse(engineConfig.selectiveBloomHideNonBloomed ? hideNonBloomed : darkenNonBloomed);
			bloomComposer.render();
			scene.traverse(engineConfig.selectiveBloomHideNonBloomed ? restoreHiddenVisibility : restoreDarkenedMaterials);
	...
	const setOptions = (nextOptions = {}) => {
		...
		if (nextOptions.selectiveBloomHideNonBloomed != null) {
			engineConfig.selectiveBloomHideNonBloomed = Boolean(nextOptions.selectiveBloomHideNonBloomed);
		}
